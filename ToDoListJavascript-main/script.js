...
function removerElemento() {
  ...
  if(minhaLista.isEmpty()) {
    ...
    msg = minhaLista.deleteFirst();
    mostrarMensagemRemocao(msg);
  }
...
function mostrarMensagemRemocao(tarefaRealizada) {
    ...
    const horasEspera = calcularDiferencaHoras(tarefaRealizada.hora, obterHoraAtual());
    mensagem.innerHTML ="Tarefa "+ tarefaRealizada.descricao + ", realizada em " + diasEspera + " Dias e " + horasEspera+ " Horas"; // apresentar a mensagem de remoção com dias e horas
    mensagem.style.display = "block";
...
function mostraPrimeiro(){
  ...
  if (minhaLista.isEmpty()) {
    ...
    mensagem.innerHTML = "Próxima: " + minhaLista.first();
    mensagem.style.display = "block";
  }
}
//-------------------------------------------------------------------------------------------- 
//Mostrar elemento a mais tempo na fila
function MaisTempo(){
  const mensagem = document.getElementById("mensagem-remocao");
  ...
  if (minhaLista.isEmpty()) {
    ...
    let itemMaisAntigo = minhaLista.first();

    minhaLista.forEach((item) => {
      if (comparaTarefasDataHora(item, itemMaisAntigo) === item) {
        itemMaisAntigo = item;
      }
    });

    const mensagem = document.getElementById("mensagem-remocao");
    const diasEspera = calcularDiferencaDias(itemMaisAntigo.data, obterDataAtual());
    const horasEspera = calcularDiferencaHoras(itemMaisAntigo.hora, obterHoraAtual());
    mensagem.innerHTML = "Item mais antigo: " + itemMaisAntigo.descricao + ", esperando há " + diasEspera + " dias e " + horasEspera + " horas";
    mensagem.style.display = "block";
